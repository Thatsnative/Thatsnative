...
public class DNSSEC {
	...
	public static class Algorithm {
		...
		/**
		 * GOST R 34.10-2001. This requires an external cryptography provider, such as
		 * BouncyCastle.
		 */
	...
	/**
	 * Creates a byte array containing the concatenation of the fields of the SIG(0)
	 * record and the message to be signed. This does not perform a cryptographic
	 * digest.
	 * 
	 * @param sig
	...
	public static class KeyMismatchException extends DNSSECException {
		...
		KeyMismatchException(KEYBase key, SIGBase sig) {
			super("key " + key.getName() + "/" + Algorithm.string(key.getAlgorithm()) + "/" + key.getFootprint()
					+ " " + "does not match signature " + sig.getSigner() + "/"
					+ Algorithm.string(sig.getAlgorithm()) + "/" + sig.getFootprint());
		}
	}
	...
	/**
	 * A DNSSEC verification failed because the cryptographic signature verification
	 * failed.
	 */
	...
	private static final ECKeyInfo GOST = new ECKeyInfo(32,
			...
			"FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFD94", "A6", "1",
	...
	private static boolean matches(SIGBase sig, KEYBase key) {
		return (key.getAlgorithm() == sig.getAlgorithm() && key.getFootprint() == sig.getFootprint()
	...
	private static byte[] sign(PrivateKey privkey, PublicKey pubkey, int alg, byte[] data, String provider)
			throws DNSSECException {
		byte[] signature;
		try {
			Signature s;
	 ...
	 */
	public static RRSIGRecord sign(RRset rrset, DNSKEYRecord key, PrivateKey privkey, Date inception, Date expiration)
			throws DNSSECException {
	 ...
	 *            The time at which the signatures should expire
	 * @param provider
	 *            The name of the JCA provider. If non-null, it will be passed to
	 *            JCA getInstance() methods.
	 * @throws UnsupportedAlgorithmException
	 *             The algorithm is unknown
	 * @throws MalformedKeyException
	 *             The key is malformed
	...
	public static RRSIGRecord sign(RRset rrset, DNSKEYRecord key, PrivateKey privkey, Date inception, Date expiration,
		...
		RRSIGRecord rrsig = new RRSIGRecord(rrset.getName(), rrset.getDClass(), rrset.getTTL(), rrset.getType(), alg,
				rrset.getTTL(), expiration, inception, key.getFootprint(), key.getName(), null);

		rrsig.setSignature(sign(privkey, key.getPublicKey(), alg, digestRRset(rrsig, rrset), provider));
		return rrsig;
	}

	static SIGRecord signMessage(Message message, SIGRecord previous, KEYRecord key, PrivateKey privkey, Date inception,
                                 Date expiration) throws DNSSECException {
		int alg = key.getAlgorithm();
		checkAlgorithm(privkey, alg);

		SIGRecord sig = new SIGRecord(Name.root, DClass.ANY, 0, 0, alg, 0, expiration, inception, key.getFootprint(),
				key.getName(), null);
		DNSOutput out = new DNSOutput();
		digestSIG(out, sig);
		if (previous != null)
	...
	static void verifyMessage(Message message, byte[] bytes, SIGRecord sig, SIGRecord previous, KEYRecord key)
			throws DNSSECException {
