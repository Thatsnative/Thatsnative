...
import static com.example.epic.model.error.HostError.DOWNLOAD_FAILED;
import static com.example.epic.model.error.HostError.NO_CONNECTION;
...
import com.example.epic.model.git.GitHostsSource;
...
public class SourceModel {
    ...
    public SourceModel(Context context) {
        ...
        this.ruleListItemDao = database.ruleListItemDao();
    ...
    private ZonedDateTime getHostsSourceLastUpdate(HostsSource source) {
        switch (source.getType()) {
            ...
            case FILE:
                if (source.getUrl().startsWith("assets://")) {
                    String assetPath = source.getUrl().substring("assets://".length());
                    return getAssetsLastUpdate(assetPath);
                } else {
    ...
    @Nullable
    private ZonedDateTime getAssetsLastUpdate(String assetPath) {
        try {
            // Возвращаем дату сборки как замену lastModified
            return ZonedDateTime.ofInstant(
                    Instant.ofEpochMilli(BuildConfig.BUILD_TIME),
                    ZoneId.systemDefault()
            );
        } catch (Exception e) {
            Timber.w(e, "Failed to get assets last update time for: %s", assetPath);
            return null;
        }
    }
    ...
    private void readSourceFile(HostsSource hostsSource) throws IOException {
        ...
        if (hostsFileUrl.startsWith("assets://")) {
            // 🔸 Специальная обработка для assets
            String assetPath = hostsFileUrl.substring("assets://".length());
            ...
            // 🔹 Всё остальное — по URI (content://, file:// и т.д.)
    ...
    private void parseSourceInputStream(HostsSource hostsSource, BufferedReader reader) {
        ...
        new SourceLoader(context, hostsSource).parse(reader, this.hostListItemDao, this.ruleListItemDao);
